...
class GUI(tk.Tk):
    def __init__(self):
        ...
        self.found_faces_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Arial italic", 50),
                                            text=" Target Faces")
        ...
        self.merged_embeddings_text = tk.Entry(self.source_button_canvas, relief='flat', bd=0,
                                               textvariable=self.merged_embedding_name)
        ...
        self.source_faces_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Arial italic", 50),
                                             text=' Source Faces')
        ...
        self.target_media_canvas.create_text(8, 45, anchor='w', fill='grey25', font=("Arial italic", 50),
                                             text=' Target Videos')
        ...
        # Label Frame 1
        self.program_options_label = tk.LabelFrame(self.program_options_frame_canvas, self.frame_style, height=42,
                                                   width=800)
    ...
    def initialize_gui(self):
        ...
        try:
            ...
            if self.json_dict["source videos"] is None:
                self.actions['LoadTVideosButton'].configure(self.button_highlight_style, text=' Setup')
            else:
        ...
        try:
            ...
            if self.json_dict["saved videos"] is None:
                self.actions['OutputFolderButton'].configure(self.button_highlight_style, text=' Setup')
            else:
        ...
        self.set_status('Welcome to Rope EX!')
    ...
    def load_source_faces(self):
        if not self.detection_model:
            ...
            if os.path.exists("merged_embeddings.txt"):
                try:
                    ...
                    for j in range(len(temp0)):
                        ...
                        self.source_faces[j]["TKButton"] = tk.Button(self.source_faces_canvas,
                                                                     self.inactive_button_style,
                                                                     image=self.pixel, text=temp0[j][0], height=14,
                                                                     width=84, compound='left')

                        self.source_faces[j]["TKButton"].bind("<ButtonRelease-1>",
                                                              lambda event,
                                                                     arg=j: self.toggle_source_faces_buttons_state(
                                                                  event, arg))
                        self.source_faces[j]["TKButton"].bind("<Shift-ButtonRelease-1>", lambda event,
                                                                                                arg=j: self.toggle_source_faces_buttons_state_shift(
                            event, arg))
                        ...
                        self.source_faces_canvas.create_window((j // 4) * 92, 8 + (22 * (j % 4)),
            ...
            if directory is None:
                print("No Source Face directory assigned")
            else:
                ...
                for i in range(len(faces)):
                    ...
                    self.source_faces[shift_i]["TKButton"] = tk.Button(self.source_faces_canvas,
                                                                       self.inactive_button_style,
                                                                       image=self.source_faces[shift_i]["Image"],
                                                                       height=86, width=86)
                    self.source_faces[shift_i]["ButtonState"] = False

                    self.source_faces[shift_i]["TKButton"].bind("<ButtonRelease-1>", lambda event,
                                                                                            arg=shift_i: self.toggle_source_faces_buttons_state(
                        event, arg))
                    self.source_faces[shift_i]["TKButton"].bind("<Shift-ButtonRelease-1>", lambda event,
                                                                                                  arg=shift_i: self.toggle_source_faces_buttons_state_shift(
                        event, arg))
                    ...
                    self.source_faces_canvas.create_window(((shift_i_len // 4) + i + 1) * 92, 8,
                                                           window=self.source_faces[shift_i]["TKButton"], anchor='nw')
            ...
            if ret:
                ...
                for face in ret:
                    ...
                    if not found:
                        ...
                        self.target_faces[last_index]["TKButton"] = tk.Button(self.found_faces_canvas,
                                                                              self.inactive_button_style, height=86,
                                                                              width=86)
                        ...
                        # Add image to button
                        self.target_faces[-1]["TKButton"].config(pady=10, image=self.target_faces[last_index]["Image"],
                                                                 command=lambda
                                                                     k=last_index: self.toggle_found_faces_buttons_state(
                                                                     k))

                        # Add button to canvas
                        self.found_faces_canvas.create_window((last_index) * 92, 8,
        ...
        if self.actions['ImgVidMode'] == 1:
            for i in range(len(images)):
                self.target_media_buttons.append(
                    tk.Button(self.target_media_canvas, self.inactive_button_style, height=86, width=86))
                ...
                self.target_media_buttons[i].config(image=self.target_media[i],
                                                    command=lambda i=i: self.load_target(i, images[i][1],
                                                                                         self.actions['ImgVidModes'][
                                                                                             self.actions[
                                                                                                 'ImgVidMode']]))
            ...
            for i in range(len(videos)):
                ...
                self.target_media.append(ImageTk.PhotoImage(image=rgb_video))
                self.target_media_buttons[i].config(image=self.target_media[i],
                                                    command=lambda i=i: self.load_target(i, videos[i][1],
                                                                                         self.actions['ImgVidModes'][
                                                                                             self.actions[
                                                                                                 'ImgVidMode']]))
                self.target_media_buttons[i].bind("<Button-5>", self.target_videos_mouse_wheel)
    ...
    def check_for_video_resize(self):
        if self.docked:
            win_geom = '%dx%d+%d+%d' % (
                self.json_dict['dock_win_geom'][0], self.json_dict['dock_win_geom'][1],
            ...
            if self.winfo_geometry() != win_geom:
                ...
                for i in range(len(self.markers)):
                    ...
                    self.video_slider_canvas.delete(self.markers[i]['icon_ref'])
                    self.markers[i]['icon_ref'] = self.video_slider_canvas.create_image(position, 30,
                                                                                        image=self.marker_icon)
            ...
            win_geom = '%dx%d+%d+%d' % (self.json_dict['undock_win_geom'][0], self.json_dict['undock_win_geom'][1],
                                        self.json_dict['undock_win_geom'][2], self.json_dict['undock_win_geom'][3])
            ...
            win_geom = '%dx%d+%d+%d' % (
                self.json_dict['player_geom'][0], self.json_dict['player_geom'][1], self.json_dict['player_geom'][2],
                self.json_dict['player_geom'][3])
    ...
    def update_CLIP_text(self, text):
        ...
        self.focus()
        print("CLIP text has been set to:", self.parameters['CLIPText'])
    ...
    def toggle_dock(self):
        if self.docked:
            ...
            self.undock_video_window.geometry('%dx%d+%d+%d' % (
                self.json_dict['player_geom'][0], self.json_dict['player_geom'][1], self.json_dict['player_geom'][2],
                self.json_dict['player_geom'][3]))

            # resize main window
            self.grid_rowconfigure(0, weight=0)
            self.geometry('%dx%d+%d+%d' % (self.json_dict['undock_win_geom'][0], self.json_dict['undock_win_geom'][1],
                                           self.json_dict['undock_win_geom'][2], self.json_dict['undock_win_geom'][3]))
    ...
    def save_selected_source_faces(self, text):
        ...
        text = text.get()
        # get embeddings from all highlighted buttons
    ...
    def create_ui_button(self, parameter, root, x, y, width=125, height=26):
        ...
        self.param_const[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.param_const[icon_holder], anchor='w',
                                             command=lambda: self.toggle_ui_button(parameter))
    ...
    def create_ui_button_2(self, parameter, root, x, y, width=96, height=26):
        ...
        if parameter == 'Dock':
            ...
            self.actions[icon_holder] = ImageTk.PhotoImage(resized_image)
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.toggle_dock())
        ...
        if parameter == 'ImgDock':
            ...
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.toggle_play_video())
            ...
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.toggle_rec_video())
            ...
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.remove_marker())
        elif parameter == 'PrevMarker':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.previous_marker())
        elif parameter == 'NextMarker':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.next_marker())
        elif parameter == 'ToggleStop':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.toggle_stop())

        elif parameter == 'FindFaces':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.add_action("find_faces", "current"))

        elif parameter == 'ClearFaces':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.add_action("clear_faces", "current"))

        elif parameter == 'SwapFaces':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.toggle_swapper())


        elif parameter == 'LoadSFaces':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.select_faces_path())
        elif parameter == 'DelEmbed':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.delete_merged_embedding())
        elif parameter == 'LoadTVideos':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.select_video_path())
        elif parameter == 'ImgVid':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.toggle_vid_img())

        elif parameter == 'StartRope':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.load_all())
        elif parameter == 'OutputFolder':
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             image=self.actions[icon_holder], anchor='w',
                                             command=lambda: self.select_save_video_path())
        ...
        # self.actions[button].bind("<MouseWheel>", self.change_video_quality)
            ...
            self.actions[button] = tk.Button(root, self.inactive_button_style, compound='left',
                                             text=self.actions['PerfTestModes'][0], image=self.actions[icon_holder],
                                             anchor='w', command=lambda: self.toggle_perf_test())
