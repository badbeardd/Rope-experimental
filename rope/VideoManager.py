...
class VideoManager():
    def __init__(self):
        ...
        self.FFHQ_kps = np.array(
            [[192.98138, 239.94708], [318.90277, 240.1936], [256.63416, 314.01935], [201.26117, 371.41043],
             [313.08905, 371.15118]])
        ...
        if not self.record and time_diff >= 1.0 / float(self.fps) and self.play:
            ...
            if index != -1:
                if self.process_qs[index]['Status'] == 'finished':
                    ...
                    if self.process_qs[index]['FrameNumber'] >= self.video_frame_total - 1 or self.process_qs[index][
                        'FrameNumber'] == self.stop_marker:
                        self.play_video('stop')
            ...
            if index != -1:
                ...
                if self.process_qs[index]['Status'] == 'finished':
                    ...
                    if self.process_qs[index]['FrameNumber'] >= self.video_frame_total - 1 or self.process_qs[index][
                        'FrameNumber'] == self.stop_marker or self.play == False:
                        self.play_video("stop")
            ...
            for fface in ret:
                for tface in self.target_facess:
                    ...
                    if sim < float(threshhold) and tface["SourceFaceAssignments"]:
                        ...
                        img = self.func_w_test("swap_video", self.swap_core, img, fface[0], s_e, parameters,
                                               frame_number)
    ...
    def swap_core(self, img, kps, s_e, parameters, frame):  # img = RGB
        ...
        original_face_512 = v2.functional.affine(img, tform.rotation * 57.2958,
                                                 (tform.translation[0], tform.translation[1]), tform.scale, 0,
                                                 center=(0, 0), interpolation=v2.InterpolationMode.BILINEAR)
        ...
        if parameters['TransformState']:
            original_face_128 = v2.functional.affine(original_face_128, 0, (0, 0),
                                                     1 + parameters['TransformAmount'][0] / 100, 0, center=(63, 63),
        ...
        for i in range(itex):
            ...
            io_binding.bind_input(name=self.input_names[0], device_type='cuda', device_id=0, element_type=np.float32,
                                  shape=(1, 3, 128, 128), buffer_ptr=prev_swap.data_ptr())
            io_binding.bind_input(name=self.input_names[1], device_type='cuda', device_id=0, element_type=np.float32,
                                  shape=(1, 512), buffer_ptr=latent.data_ptr())
            io_binding.bind_output(name=self.output_names[0], device_type='cuda', device_id=0, element_type=np.float32,
                                   shape=tuple(swap.shape), buffer_ptr=swap.data_ptr())
        ...
        gauss = transforms.GaussianBlur(parameters['BorderAmount'][3] * 2 + 1,
                                        (parameters['BorderAmount'][3] + 1) * 0.2)
        ...
        if parameters["CLIPState"]:
            ...
            with lock:
                mask = self.func_w_test('CLIP', self.apply_neg_CLIPs, inface, parameters["CLIPText"],
                                        parameters["CLIPAmount"][0])
        ...
        if parameters["FaceParserState"]:
            ...
            mask2 = self.func_w_test('mouth parser', self.apply_face_parser, original_face_512,
                                     parameters["FaceParserAmount"][0])
        ...
        if not parameters['MaskViewState']:
            ...
            swap = v2.functional.affine(swap, tform.inverse.rotation * 57.2958,
                                        (tform.inverse.translation[0], tform.inverse.translation[1]),
                                        tform.inverse.scale, 0, interpolation=v2.InterpolationMode.BILINEAR,
                                        center=(0, 0))
            ...
            swap_mask = v2.functional.affine(swap_mask, tform.inverse.rotation * 57.2958,
                                             (tform.inverse.translation[0], tform.inverse.translation[1]),
                                             tform.inverse.scale, 0, interpolation=v2.InterpolationMode.BILINEAR,
                                             center=(0, 0))
    ...
    def apply_occlusion(self, img, amount):
        ...
        io_binding.bind_input(name='img', device_type='cuda', device_id=0, element_type=np.float32,
                              shape=(1, 3, 256, 256), buffer_ptr=img.data_ptr())
        io_binding.bind_output(name='output', device_type='cuda', device_id=0, element_type=np.float32,
                               shape=(1, 1, 256, 256), buffer_ptr=outpred.data_ptr())
    ...
    def apply_face_parser(self, img, FaceParserAmount):
        ...
        if FaceParserAmount != 0:
            ...
            io_binding.bind_input(name='input', device_type='cuda', device_id=0, element_type=np.float32,
                                  shape=(1, 3, 512, 512), buffer_ptr=img.data_ptr())
            io_binding.bind_output(name='out', device_type='cuda', device_id=0, element_type=np.float32,
                                   shape=(1, 19, 512, 512), buffer_ptr=outpred.data_ptr())
    ...
    def apply_GFPGAN(self, swapped_face_upscaled, parameters):
        ...
        temp = v2.functional.affine(swapped_face_upscaled, tform.rotation * 57.2958,
                                    (tform.translation[0], tform.translation[1]), tform.scale, 0, center=(0, 0))
        ...
        io_binding.bind_input(name='input', device_type='cuda', device_id=0, element_type=np.float32,
                              shape=(1, 3, 512, 512), buffer_ptr=temp.data_ptr())
        io_binding.bind_output(name='output', device_type='cuda', device_id=0, element_type=np.float32,
                               shape=(1, 3, 512, 512), buffer_ptr=outpred.data_ptr())
        ...
        # Invert Transform
        outpred = v2.functional.affine(outpred, tform.inverse.rotation * 57.2958,
                                       (tform.inverse.translation[0], tform.inverse.translation[1]), tform.
    ...
    def apply_codeformer(self, swapped_face_upscaled, parameters):
        ...
        io_binding.bind_input(name='x', device_type='cuda', device_id=0, element_type=np.float32,
                              shape=(1, 3, 512, 512), buffer_ptr=temp.data_ptr())
        io_binding.bind_cpu_input('w', w)
        io_binding.bind_output(name='y', device_type='cuda', device_id=0, element_type=np.float32,
                               shape=(1, 3, 512, 512), buffer_ptr=outpred.data_ptr())
    ...
    def ret50_landmarks(self, image):
        ...
        io_binding.bind_input(name='input', device_type='cuda', device_id=0, element_type=np.float32,
                              shape=(1, 3, 512, 512), buffer_ptr=image.data_ptr())
        io_binding.bind_output(name='conf', device_type='cuda', device_id=0, element_type=np.float32,
                               shape=(1, 10752, 2), buffer_ptr=conf.data_ptr())
        io_binding.bind_output(name='landmarks', device_type='cuda', device_id=0, element_type=np.float32,
                               shape=(1, 10752, 10), buffer_ptr=landmarks.data_ptr())
        ...
        tmp = (priors[:, :2] + pre[:, :2] * 0.1 * priors[:, 2:], priors[:, :2] + pre[:, 2:4] * 0.1 * priors[:, 2:],
               priors[:, :2] + pre[:, 4:6] * 0.1 * priors[:, 2:], priors[:, :2] + pre[:, 6:8] * 0.1 * priors[:, 2:],
